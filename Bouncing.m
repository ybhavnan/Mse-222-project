...
%Time: The total time the ball takes to bounce in seconds. This one apply
%to the right side ramp
% angle is angle of the ramp and c is coefficient of restituition
global GlobalXYT;
g=9.81;
localtime = 0;
...
Vx = abs(((GlobalXYT(end,1) - GlobalXYT(end-2,1))/(GlobalXYT(end,3) - GlobalXYT(end-2,3))))*c;%velocity after impact in x direction
Vy = abs(((GlobalXYT(end,2) - GlobalXYT(end-2,2))/(GlobalXYT(end,3) - GlobalXYT(end-2,3))))*c;% velocity after impact in y -direction
...
while (stop == false)
    ...
    if ((bounce_angle - angle) <= 0)% this to check if the ball gonna bounce again or not
        stop = true;
