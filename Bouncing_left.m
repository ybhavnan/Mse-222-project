function[result] = Bouncing_left(angle, c) % this is the same as bouncing motion, however it applies for the left side ramp
global GlobalXYT;
g=9.81;
localtime = 0;
...
Vx = -c*abs(((GlobalXYT(end,1) - GlobalXYT(end-1,1))/(GlobalXYT(end,3) - GlobalXYT(end-1,3))));%velocity after impact in x direction
Vy = c*abs(((GlobalXYT(end,2) - GlobalXYT(end-1,2))/(GlobalXYT(end,3) - GlobalXYT(end-1,3))));% velocity after impact in y -direction
...
bounce_angle= acot(abs((Vx/Vy)));%Contact angle change after each bounce.
